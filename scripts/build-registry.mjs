...
// Consume with:  npx shadcn add ./public/r/<name>.json
...
write("foundation", {
  ...
  description:
    ...
    IMPORT_NOTE("atrium-ui/foundation.css"),
  files: [
    {
      path: "app/atrium-ui/foundation.css",
      content: foundationCss,
      type: "registry:file",
      target: "app/atrium-ui/foundation.css",
    },
  ],
});
...
for (const [name, meta] of Object.entries(INTERNAL)) {
  write(name, {
    ...
    type: meta.type,
    title: meta.title,
    description: `Shared building block used by other Atrium UI components.`,
...
for (const [name, title, file, deps, npm, extraCss] of PRIMITIVES) {
  ...
  if (extraCss) {
    const cssTarget = `app/atrium-ui/${name}.css`;
    files.push({ path: cssTarget, content: extraCss, type: "registry:file", target: cssTarget });
    description += IMPORT_NOTE(`atrium-ui/${name}.css`);
  }
...
write("registry", {
  ...
  homepage: ".",
  items: [
    { name: "foundation", type: "registry:style", title: "Atrium UI foundation" },
