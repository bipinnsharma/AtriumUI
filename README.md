...
> **Icons.** `SidebarNav` uses [`lucide-react`](https://lucide.dev) (MIT, no
> license key needed). Everything else runs with no configuration.
...
```bash
npx shadcn add ./public/r/approval-card.json
```

Browse the index at
[`/r/registry.json`](./public/r/registry.json). Each item
