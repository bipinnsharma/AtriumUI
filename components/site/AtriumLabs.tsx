...
import MessagesView, { AiAssistantPanel } from "@/components/primitives/MessagesView";
...
import PatientRecordsTable from "@/components/primitives/PatientRecordsTable";
...
/* the clinical cases flow: answer the questions, the card collapses to a small
...
function OffboardingAnswer() {
  ...
  return (
    <>
      <Reply intro="Before I pull up the case files, confirm a few details. Single questions advance on their own; multi-selects wait for the arrow.">
      ...
      {stage === "thinking" && (
        <div className="mt-4 flex min-h-6 items-center" style={{ animation: "fade-in 200ms ease-out both" }}>
          <LoadingState label="Loading cases" variant="Dots" />
        </div>
      )}
      {stage === "done" && (
        <div className="mt-4" style={{ animation: "fade-up 400ms cubic-bezier(0.23,1,0.32,1) both" }}>
          <StreamLine text="Done — I pulled 7 cases from this week. Three need follow-up notes, two are ready for discharge, and the attending wants an update on Bed 5A by noon." />
        </div>
      )}
    </>
  );
    ...
    <>
      ...
      {settled && (
        <div className="mt-1" style={{ animation: "fade-in 200ms ease-out both" }}>
          <StreamLine tone="ink-2" text="Found it — the relevant papers and clinical guidelines, plus the two docs it references. The retrieved chunks are in the side panel." />
        </div>
      )}
    </>
  );
...
const SCENARIOS: Record<string, Scenario> = {
  "patient-rounds": {
    prompt: "Summarize today's patient round.",
    beat: 900,
    Answer: () => <WorkloadAnswer />,
  },
  "drug-interactions": {
    prompt: "Check drug interactions for Metformin and Lisinopril.",
    beat: 500,
    paneTitle: "Context",
    Pane: () => <ContextCards />,
  ...
  "clinical-cases": {
    prompt: "Show me my clinical cases for this week.",
    beat: 850,
    Answer: () => <OffboardingAnswer />,
  },
  research: {
    prompt: "What papers should I read on cardiology?",
    beat: 500,
    paneTitle: "Context",
    Pane: () => <ContextCards />,
    Answer: () => <FindTicketAnswer />,
  ...
  notes: {
    prompt: "Draft a discharge summary for Patient #4821.",
    beat: 700,
    Answer: () => (
  ...
  guidelines: {
    prompt: "What's the latest protocol for managing Type 2 Diabetes?",
    beat: 500,
    Answer: () => (
      <Reply intro="I pulled the latest guidelines and staged the edits. Hover a chip to preview — nothing runs until you say so.">
        <ToolChips />
      </Reply>
    ),
  },
  "imaging-orders": {
    ...
    beat: 700,
    Answer: () => (
      <Reply intro="Here are the current imaging orders across all departments.">
        <PatientRecordsTable />
      </Reply>
    ),
  },
...
const KEYWORDS: [ScenarioId, string[]][] = [
  ["patient-rounds", ["round", "rounds", "patient", "vitals", "ward", "summary", "today"]],
  ["drug-interactions", ["drug", "interaction", "medication", "metformin", "lisinopril", "prescription"]],
  ["clinical-cases", ["case", "clinical", "cases", "specialty", "admission", "discharge"]],
  ["research", ["paper", "research", "read", "cardiology", "journal", "study"]],
  ["todos", ["todo", "to-do", "task", "need", "today", "urgent"]],
...
function SuggestionIcon({ kind }: { kind: string }) {
  const paths: Record<string, ReactNode> = {
    ...
    "patient-rounds": <g><rect x="3" y="4" width="18" height="16" rx="2" /><path d="M8 10l2.5 2.5L16 8" /></g>,
    "drug-interactions": <g><circle cx="11" cy="11" r="7" /><path d="M21 21l-4.3-4.3" /></g>,
    "clinical-cases": <g><rect x="3" y="3" width="7" height="7" rx="1.5" /><rect x="14" y="3" width="7" height="7" rx="1.5" /><rect x="3" y="14" width="7" height="7" rx="1.5" /><rect x="14" y="14" width="7" height="7" rx="1.5" /></g>,
    research: <path d="M8 6l-5 6 5 6M16 6l5 6-5 6" />,
    notes: <g><path d="M12 20h9" /><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4z" /></g>,
    guidelines: <g><path d="M4 6h10M18 6h2M4 12h2M10 12h10M4 18h14M20 18h0" /><circle cx="16" cy="6" r="2" /><circle cx="8" cy="12" r="2" /><circle cx="18" cy="18" r="2" /></g>,
    "imaging-orders": <g><rect x="3" y="3" width="18" height="18" rx="2" /><path d="M3 9h18M9 3v18" /></g>,
  };
  return (
    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
      {paths[kind] ?? paths.todos}
    </svg>
  );
}

const SUGGESTION_POOL: { id: ScenarioId; label: string }[] = [
  { id: "patient-rounds", label: "Summarize today's patient round" },
  { id: "todos", label: "What do I need to do today?" },
  { id: "drug-interactions", label: "Check drug interactions for Metformin" },
  { id: "research", label: "What papers should I read on cardiology?" },
  { id: "notes", label: "Draft a discharge summary for Patient #4821" },
  { id: "guidelines", label: "What's the latest protocol for Type 2 Diabetes?" },
  { id: "imaging-orders", label: "Show me the current imaging orders" },
];

const RECENTS: { id: ScenarioId; label: string; prompt?: string }[] = [
  { id: "patient-rounds", label: "Patient rounds — Ward 3B" },
  { id: "drug-interactions", label: "Drug interaction check" },
  { id: "clinical-cases", label: "Clinical cases this week" },
  { id: "research", label: "Research papers on cardiology" },
  { id: "todos", label: "Daily tasks" },
      ...
      <div className="home-reveal relative mt-7" style={homeRevealStyle(stage >= 3, revealParams.reveal)}>
        <div className="relative">
          <PromptBar
            demo={false}
            tall
            placeholder="Ask anything about your patients, research, or clinical data…"
            defaultText="Summarize today's patient round"
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col" style={{ animation: "fade-in 160ms ease-out both" }}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4">
        <div className="text-[14px] font-semibold text-ink">{view} records</div>
        ...
        <div className="mt-4 rounded-[10px] bg-inset p-3 text-[13px] leading-relaxed text-ink-2 shadow-hairline">
          Only surface <span className="rounded-[5px] bg-accent-tint px-1.5 py-0.5 text-[12px] font-medium text-accent-ink">{view}</span> records with a strong, recent connection — nothing else.
        </div>
          ...
          {workspaceScenario ? (
            <>
              ...
              <section className="flex min-w-0 flex-1 flex-col overflow-hidden rounded-[14px] border border-line bg-page">
                ...
                <div className="flex h-10 shrink-0 items-center gap-1 overflow-x-auto rounded-[14px] border border-line px-2">
              ...
              <aside
                className="hidden w-[400px] shrink-0 flex-col overflow-hidden border border-line bg-page lg:flex"
