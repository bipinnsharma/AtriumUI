 ...
 *   Surfer — the Drive loader paired with a meme gif below
 *
...
function LoaderGrid({
  ...
  return (
    <span aria-hidden className="grid shrink-0 grid-cols-[repeat(3,2px)] gap-[2px]">
      {delays.map((delay, index) => (
        <span
          key={index}
          className={`size-[2px] bg-ink ${round ? "rounded-full" : "rounded-[1px]"}`}
          style={{
...
export default function LoadingState({
  label,
  variant = "Drive",
  /** the meme feed for the Surfer variant; dancing baby gif */
  videoSrc = "https://neal.fun/internet-artifacts/dancing-baby.gif",
}: {
  label?: string;
  variant?: string;
  videoSrc?: string;
  ...
  const resolvedLabel = label ?? (surfer ? "Dancing baby" : "Churning");
