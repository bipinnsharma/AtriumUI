...
const ROWS: DiffRow[] = [
  { key: "aspirin", id: "Aspirin 81mg", dept: "Cardiac", email: "current-med-list", removed: true },
  { key: "ibuprofen", id: "Ibuprofen 400mg", dept: "NSAID", email: "current-med-list", removed: true },
  { key: "metformin", id: "Metformin 500mg", dept: "Diabetes", email: "new-protocol", removed: false },
];
...
export default function DiffTable({
  ...
  const [edits, setEdits] = useState<Record<string, boolean>>({ aspirin: true, ibuprofen: true, metformin: true });

  const removals = ["aspirin", "ibuprofen"].filter((key) => edits[key]).length;
  const additions = edits.metformin ? 1 : 0;
    ...
    <div className="w-full max-w-95">
      <div className="relative overflow-hidden rounded-card bg-surface shadow-card">
        <div className="primitive-card-bar flex items-center justify-between border-b border-line">
          <span className="text-[12.5px] font-medium text-ink">Proposed medication changes</span>
        ...
        <table className="w-full table-fixed border-collapse text-left">
          ...
          <thead>
            <tr className="border-b border-line">
              {["Medication", "Category", "Source"].map((h) => (
                ...
                >
                  <div className="overflow-hidden">
                    <div
                      ...
                      aria-checked={edits.metformin}
                      aria-label="Include adding Metformin"
                      onClick={accepted ? undefined : () => toggleEdit("metformin")}
                      onKeyDown={accepted ? undefined : (event) => {
                        if (event.key === "Enter" || event.key === " ") {
                          event.preventDefault();
                          toggleEdit("metformin");
                        }
                      }}
                      ...
                      style={{ background: edits.metformin ? "var(--green-tint)" : undefined }}
                    >
                      <span className="primitive-table-cell text-[13px] font-medium tabular-nums transition-colors duration-200" style={{ color: edits.metformin ? "var(--green)" : "var(--ink-3)" }}>
                        Metformin
                      </span>
                      <span className="primitive-table-cell">
                        <span className="inline-flex h-5.5 items-center gap-1.5 rounded-full bg-surface px-2 text-[11.5px] font-medium shadow-hairline">
                          ...
                          <span className="text-ink-2">Diabetes</span>
                        </span>
                      </span>
                      <span className="primitive-table-cell text-[13px] transition-colors duration-200" style={{ color: edits.metformin ? "var(--green)" : "var(--ink-3)" }}>
                        <span className="flex items-center justify-between gap-2">
                          <span className="min-w-0 truncate">new-protocol</span>
                          <IncludedMark included={edits.metformin} tone="green" />
                        </span>
                      </span>
                    </div>
                  </div>
