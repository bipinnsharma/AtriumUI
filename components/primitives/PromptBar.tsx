...
const SOURCES: Source[] = [
  ...
  { key: "emr", name: "EMR System", desc: "Patient records & vitals", glyph: "chart" },
  { key: "research", name: "Research papers", desc: "26 papers, tags, links", glyph: "layers" },
...
const COMMANDS = [
  { key: "compare", name: "/compare", desc: "Patient vs. last visit" },
  { key: "protocol", name: "/protocol", desc: "Draft a treatment plan" },
  { key: "discharge", name: "/discharge", desc: "Build a discharge summary" },
  { key: "draft-note", name: "/draft-note", desc: "Write a clinical note" },
  { key: "summarize", name: "/summarize", desc: "Digest the thread so far" },
];

const MODELS = [
  { key: "clinical-5", name: "ClinicalGPT 5", tag: "Flagship" },
  { key: "medbase-1", name: "MedBase 1", tag: "Basic" },
  { key: "legacy-04", name: "Legacy 0.4", tag: "Deprecated" },
];

const FILES = ["vitals-chart.png", "lab-results.pdf", "emr-export.csv"];
...
const AUTO_STEPS: {
  ...
  { draft: "", connect: false, model: "medbase-1", hold: 1100 },
  ...
  { draft: "", model: "clinical-5", hold: 2400 },
  { draft: "", hold: 900 },
];
...
export default function PromptBar({
  ...
  const selectModel = (next: (typeof MODELS)[number]) => {
    setModel(next);
    setModelOpen(false);
    if (next.key === "clinical-5") celebrate();
  };
      ...
      >
        ...
        <div
          ...
          className={`grid items-end gap-x-1 gap-y-1.5 ${
            wide
              ? "grid-cols-[28px_auto_minmax(0,1fr)_auto]"
        ...
        >
          ...
          <textarea
            ...
            aria-label="Prompt"
            disabled={listening}
            className={`${tall ? "px-2 py-2 text-[14px] leading-5" : "min-h-7 px-1 py-[5px] text-[13px] leading-[18px]"} min-w-0 w-full resize-none bg-transparent text-ink outline-none [overflow-wrap:anywhere] placeholder:text-ink-3 ${listening ? "opacity-50" : ""} ${
          ...
          {/* dictation / send — replaced by recording UI when listening */}
          {listening ? (
            <div className={`flex items-center h-7 w-35  pl-3 gap-1 rounded-control   ${wide ? "col-start-4 col-span-2 row-start-2" : "col-start-4 col-span-2 row-start-1"}`}>
              <div className="items-center flex basis-[0%] grow justify-center mask-[linear-gradient(90deg,rgba(0,0,0,0),rgb(0,0,0)_6px,rgb(0,0,0)_calc(100%-6px),rgba(0,0,0,0))] min-w-0 overflow-clip gap-0.5">
                {[
                  "h-[4.56847px]", "h-[4.77574px]", "h-[4.91636px]", "h-0.75",
                  "h-2", "h-[4.21599px]", "h-1.75", "h-[3.96921px]",
                  "h-3.25", "h-[3.86627px]", "h-1.75", "h-[4.63189px]",
                  "h-[4.6273px]", "h-[4.44255px]", "h-[4.26149px]", "h-[4.30607px]",
                ].map((h, i) => (
                  <div
                    key={i}
                    className={`w-[2.5px] rounded-full shrink-0 bg-[#FF420699] ${h}`}
              ...
              <button
                ...
                className={`items-center flex justify-center p-1.5 overflow-clip gap-1 shrink-0 border border-solid border-transparent size-7 text-ink-3 transition-[background-color,color] duration-150 hover:bg-hover hover:text-ink ${pill ? "rounded-full" : "rounded-[8px]"}`}
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" style={{ height: 14, flexShrink: 0, width: 14 }}>
                  <path d="M18 6 6 18" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                  <path d="m6 6 12 12" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </button>
              <button
                type="button"
                ...
                className={`items-center flex justify-center shrink-0 size-7 transition-[background-color] duration-150 hover:bg-ink ${pill ? "rounded-full" : "rounded-[8px]"}`}
                style={{ background: "var(--ink)", color: "var(--surface)" }}
              >
                <svg width="16" height="16" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" style={{ height: 16, width: 16, flexShrink: 0 }}>
                  <path d="M19.8 6.6L10.122 19.504L4.308 13.922L5.692 12.479L9.878 16.496L18.2 5.4L19.8 6.6Z" fill="currentColor" />
                </svg>
              </button>
            </div>
          ) : (
            <>
              ...
              >
                <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" style={{ flexShrink: 0 }}>
                  <path d="M2.402 9.258C2.628 9.05 2.979 9.064 3.187 9.29L3.288 9.399C5.742 12.067 9.966 12.021 12.361 9.3C12.564 9.07 12.915 9.047 13.145 9.249C13.375 9.452 13.398 9.804 13.195 10.034C11.918 11.485 10.2 12.289 8.433 12.449C8.44 12.484 8.445 12.519 8.445 12.556V14.333C8.445 14.64 8.196 14.889 7.889 14.889C7.583 14.889 7.334 14.64 7.334 14.333V12.556C7.334 12.525 7.337 12.495 7.341 12.466C5.551 12.358 3.792 11.59 2.469 10.152L2.369 10.043C2.161 9.817 2.176 9.466 2.402 9.258ZM7.889 0.889C9.669 0.889 11.111 2.332 11.111 4.111V7.223C11.111 9.002 9.669 10.444 7.889 10.444C6.11 10.444 4.667 9.002 4.667 7.223V4.111C4.667 2.332 6.11 0.889 7.889 0.889ZM7.889 2C6.723 2 5.778 2.945 5.778 4.111V7.223C5.778 8.388 6.724 9.333 7.889 9.333C9.055 9.333 9.999 8.388 10 7.223V4.111C10 2.945 9.055 2 7.889 2Z" fill="currentColor" />
                </svg>
              </button>
