...
const TOKENS: StreamingToken[] = [
  ..."Patient #1247 in Bed 3B showed improvement — vitals stable, temperature back to normal. Bed 5A needs attention, the culture results are pending and the attending wants an update by noon."
    .split(" ")
    .map((text) => ({ text })),
  { text: "", cite: true },
  ..."Ward 3B overall occupancy is at 78% with two discharges expected today."
    .split(" ")
    .map((text) => ({ text })),
];

const FOLLOW_UPS = [
  "Which patients need follow-up notes?",
  "Compare this week's discharge rate to last week",
];
...
const SOURCES: StreamingSource[] = [
  { name: "EMR System", domain: "hospital-emr.org", href: "https://hospital-emr.org/", image: SOURCE_IMAGES.emr },
  { name: "Lab Results", domain: "labs.hospital.org", href: "https://labs.hospital.org/", image: SOURCE_IMAGES.labs },
  { name: "Rounds Data", domain: "rounds.hospital.org", href: "https://rounds.hospital.org/", image: SOURCE_IMAGES.rounds },
];
      ...
      <p className="text-[13px] leading-relaxed text-ink">
        {content.slice(0, count).map((token, i) =>
          token.cite ? (
            ...
            <span key={i} className="inline" style={{ animation: "stream-word 180ms cubic-bezier(0.23,1,0.32,1) both" }}>
              {token.text}{" "}
            </span>
          ),
        )}
