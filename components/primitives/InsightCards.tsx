...
const ALLOCATION_SEGMENTS: AllocationSegment[] = [
  { name: "CARD", label: "Cardiology", pct: 72.5, amount: "142 patients", cls: "bg-orange", tone: "text-orange" },
  { name: "NEURO", label: "Neurology", pct: 22.8, amount: "45 patients", cls: "bg-green", tone: "text-green" },
  { name: "ORTHO", label: "Orthopedics", pct: 4.7, amount: "9 patients", cls: "bg-line", tone: "text-ink-3" },
];
...
const PAGES: InsightPage[] = [
  {
    ...
    prose: (
      <>
        The most critical patient in your <Entity name="Ward" tone="bg-orange" /> is
        Bed 5A — temperature <Mono tone="red">+2.1°C</Mono> above baseline with <Mono tone="red">elevated WBC</Mono>.
      </>
    ),
    Card: CompareCard,
    pill: "Should I escalate to attending?",
  },
  {
    key: "anomaly",
    prose: (
      <>
        Unusually high bed occupancy on <span className="font-medium text-ink">Dec 13</span> —{" "}
        <Mono tone="red">+18%</Mono> above your average.
      </>
    ),
    Card: AnomalyCard,
    pill: "Get tips on reducing length of stay",
  },
  {
    key: "allocation",
    prose: (
      <>
        You’re heavily invested in <Entity name="Cardiology" tone="bg-orange" /> — it’s{" "}
        <span className="font-medium text-ink">72.5%</span> of your case.
      </>
    ),
    Card: AllocationCard,
    pill: "How should I rebalance the service lines?",
  },
];
    ...
    <div className="min-h-[408px] w-full max-w-86">
      ...
      <div className="flex items-center justify-between">
        <span className="flex items-baseline gap-1.5">
          ...
          <span className="text-[13px] text-ink-3 tabular-nums">{page + 1}/{pages.length}</span>
        </span>
